...
export type DeviceFrameProps = {
  variant?: "phone" | "landscape" | "composition";
  children: ReactNode;
  className?: string;
};
...
export function DeviceFrame({
  ...
  if (variant === "phone") {
    return (
      <div className={cn("relative mx-auto w-full max-w-[300px] sm:max-w-[340px]", className)}>
        ...
        >
          <div className="overflow-hidden rounded-[1.45rem] bg-[#050B14] ring-1 ring-white/10">
            {children}
          </div>
        </div>
      </div>
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0A1624]/80 p-2 sm:p-3 backdrop-blur-sm",
